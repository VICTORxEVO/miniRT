...
double	**create_matrix_3x3(double a, double b, double c, double l, double m,
...
double	**create_matrix_4x4(double a, double b, double c, double d, double e,
...
double	get_minor_n(double **matrix, int row, int col, int n)
{
	...
	sub = submatrix(matrix, row, col, n);
	minor = get_determinant_n(sub, n - 1);
	return (minor);
}
...
double	get_cofactor_n(double **matrix, int row, int col, int n)
{
	...
	minor = get_minor_n(matrix, row, col, n);
	cofactor = minor;
	if ((row + col) % 2 != 0)
	{
		cofactor *= -1;
...
t_ray	transform_ray(t_ray old_r, double **transformation_mx)
{
	...
	origin_mul_res_matrix = mul_matrix_row_col(transformation_mx,
			ray_matrix_origin, 4, 1);
	...
	direction_mul_res_matrix = mul_matrix_row_col(transformation_mx,
			ray_matrix_direction, 4, 1);
